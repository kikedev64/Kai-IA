...
export function registerConfigIpc(): void {
  ...
  ipcMain.handle('config:set-backend', async (_event, payload: { key: string; value: string }) => {
    const baseUrl = await getBackendBaseUrl()
  ...
  ipcMain.handle('config:set-many-backend', async (_event, entries: Record<string, string>) => {
    const baseUrl = await getBackendBaseUrl()
